...
def update_database():
    global entering, exiting, vagon
    entering_count = len(entering)
    exiting_count = len(exiting)
    
    # Sadece giren veya çıkan sayısı değiştiğinde veritabanını güncelle
    if entering_count != previous_entering_count or exiting_count != previous_exiting_count:
        insert_data(entering_count, exiting_count, vagon)
        # Veritabanına eklenen verileri ekrana yazdır
        print(f"Giren: {entering_count}, Çıkan: {exiting_count}, Vagon: {vagon}")
    
    # Önceki giren ve çıkan sayılarını güncelle
    previous_entering_count = entering_count
    previous_exiting_count = exiting_count
